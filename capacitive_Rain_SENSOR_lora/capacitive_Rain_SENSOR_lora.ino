...
int calculateMaxChange() {
  int maxValue = sensorvalues[0];
  int minValue = sensorvalues[0];
  for (int i = 0; i < 40; i++) {
...
delay(1000);
  ...
  soilval=soilval/40;
   int maxChange = calculateMaxChange();
 Serial.println("sens"); 
Serial.println(soilval);
 Serial.println("max"); 
Serial.println(maxChange);
      uint8_t payload[4];
      uint16_t valu=(uint16_t)(soilval);
      uint16_t mx=(uint16_t)(maxChange);
